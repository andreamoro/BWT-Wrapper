...
def account(fake_api):
    """An Account whose transport is replaced with the FakeApi fixture."""
...
def test_webproperties_returns_webproperty_objects(account):
...
def test_webproperties_are_cached(account, fake_api):
    account.webproperties()
    account.webproperties()
    account[0]
    # Despite multiple accesses, the API is only hit once.
    assert fake_api.get_sites_calls == 1


def test_index_by_int(account):
    assert account[1].url == "https://blog.example.com/"


def test_index_by_exact_url(account):
    assert account["https://example.com"].is_verified is True


def test_index_by_url_is_trailing_slash_and_case_insensitive(account):
...
def test_unknown_url_raises_keyerror(account):
    with pytest.raises(KeyError):
        account["https://missing.example.com"]


def test_invalid_index_type_raises_typeerror(account):
    with pytest.raises(TypeError):
        account[1.5]


def test_len_and_iter(account):
